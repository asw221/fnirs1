...
double dlm_forward_filter_draw(REP *rep,sDLM *dlmStruc,double *d,double *n,const int Pmax, const int P, const double beta,const double delta,double *W)
{
    double n0,S0;
    double Q,*A;
    double f,e;
...
void dlm_forward_filter(REP *rep,sDLM *dlmStruc,double *d,double *n)
{
    int P;
    double n0,S0;
    double Q,*A;
...
void dlm_backward_smoothing(REP *rep,sDLM *dlmStruc,const int P,double *n,unsigned long *seed)
{
 //   int n;
    double *m,*Var;
...
//    calculate_res3(rep);
    ...
    old_loglik = dlm_forward_filter_draw(rep,dlmStruc,&d,&n,(const int)Pmax,(const int)P,(const double)rep->df_delta2,(const double)rep->df_delta1,W);
    ...
    new_loglik = dlm_forward_filter_draw(rep,dlmStruc,&d,&n,(const int)Pmax,(const int)prop_P,(const double)prop_beta,(const double)prop_delta,W);
...
void DLM(REP *rep,unsigned long *seed) {
 ...
 //   calculate_res3(rep,P);
    ...
    dlm_forward_filter(rep,dlmStruc,&d,&n);
    ...
    dlm_backward_smoothing(rep,dlmStruc,(const int)P,&n,seed);
