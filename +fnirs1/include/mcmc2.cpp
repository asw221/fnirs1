...
//printf("iter = %d %d\n",iter,rep->nKnots-2*sdegree);fflush(stdout);
                if (rep->dim_W[1] > 0) {
                  ...
                  //  }
                  //  if ((iter>(BURN_IN)/2) && (iter<=BURN_IN)) {
                    //    rep->md1 += rep->df_delta1;
                    //    rep->md2 += rep->df_delta2;
                  //      rep->tableP[rep->P]++;
                  //      fprintf(rep->fout_dlm,"%lf %lf %d\n",rep->df_delta1,rep->df_delta2,rep->P);
                  //  }
                  /*  if (iter == BURN_IN) {
                       // rep->md1 /= (double)(BURN_IN/2);
                       // rep->md2 /= (double)(BURN_IN/2);
                        int imax = -1;
                        int j = 0;
                        fprintf(flog,"\n");
                        for (int i=0;i<=maxP;i++) {
                            fprintf(flog,"%d ",rep->tableP[i]);                          
                            if (rep->tableP[i] > imax) {
                                imax = rep->tableP[i];
                                j = i;
                            }
                        }
                        ...
                        fclose(rep->fout_dlm);
                        rep->df_delta1 = rep->md1/(double)cntj;
                        rep->df_delta2 = rep->md2/(double)cntj;
                        rep->P = j;
                        rep->dim_W[1] = rep->P;
...
void calH(double *H,double *A,double *m,const int nrow,const int ncol,const int P) {
    ...
    for (t=P;t<nrow;t++) 
        for (j=0;j<P;j++) 
            for (k=0;k<ncol;k++) 
//                H[t*ncol+k] += A[(t-j-1)*ncol+k]*dlmStruc[t-1].m[j];
...
void calculate_res5(REP *rep,double *Wm,int P) {
    for (int i=0;i<P;i++) {
        rep->residuals5[i] = rep->Y[i];
    }
   for (int i=P;i<rep->dim_V[0];i++)
        rep->residuals5[i] = rep->Y[i] - Wm[i];
...
void draw_beta_eta(POP *pop,SUB *sub,REP *rep,unsigned long *seed,int iter,int isub) {
    ...
    void dlm_forward_filter2(REP *,sDLM *);
    
    calW(rep->W,rep->Y,rep->Xbeta,rep->Veta,(const int)rep->dim_W[0],(const int)rep->dim_W[1],rep->P);
    dlm_forward_filter2(rep,dlmStruc);
    ...
    calH(J,X,rep->m,(const int)rep->dim_V[0],(const int)ncol,(const int)rep->P);
    ...
    calW2(rep->W,rep->Y,(const int)rep->dim_W[0],(const int)rep->dim_W[1],rep->P);
    calWdelta(rep->Wm,rep->W,rep->m,(const int)rep->dim_W[0],(const int)rep->dim_W[1]);
    calculate_res5(rep,rep->Wm,rep->P);
    ...
    double *Qinv = (double *)calloc(rep->dim_V[0],sizeof(double));
    for (i=0;i<rep->P;i++)
        Qinv[i] = rep->d_Y[i];
    for (i=rep->P;i<rep->dim_V[0];i++)
        Qinv[i] = 1./dlmStruc[i].Q;
    calApVinvX(M,V,rep->residuals5,Qinv,rep->dim_V[0],ncol);
 
     /* calculate  (X - J)'(X - J) */
    
    calApVinvA(VpV,V,Qinv,(const int)rep->dim_V[0],(const int)ncol);
...
//    calculate residuals with Wm
//    calculate p(Y|everything) Importance density
//    calculate_marginal_residuals(rep,rep->P);
    double importance_dens = 0;
    for (i=0;i<rep->dim_X[0];i++) {
        importance_dens += log(Qinv[i]) - Qinv[i]*rep->residuals[i]*rep->residuals[i];
//         printf("imp = %lf Qinv = %g res = %g\n",importance_dens,Qinv[i],rep->residuals[i]);
    }
 //   printf("imp = %g\n",importance_dens);
    importance_dens *= 0.5;
//exit(0);
...
//      calculate W and Q_t and m_t (dlm_forward_filter2)
//    calculate p(Y| everything) true density
//    multi mean*true/importance
    ...
    double true_dens = 0;
    for (i=0;i<rep->dim_X[0];i++)
        true_dens += log(Qinv[i]) - Qinv[i]*rep->residuals[i]*rep->residuals[i];
    true_dens *= 0.5;

    if (iter >0){
    double ratio = exp(-true_dens + importance_dens);
//    printf("ratio = %g %f %f\n",ratio,true_dens,importance_dens);fflush(stdout);
    for (i=0;i<rep->dim_X[1];i++)
        sub->beta[i] *= ratio;
    for (i=0;i<rep->dim_V[1];i++)
        rep->eta[i] *= ratio;
